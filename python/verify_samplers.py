...
exec(open("python/verify_settings.py").read())
...
if true_y:
    ...
    exec(open("python/verify_lib.py").read())
...
_, samps, switch_rates = bayes_trans(Xs, ys, iters = iters, fixed = fixed, debug = False, plotprefix = f"debug/{seed}", omega_prior = omega_prior, rb = rb, L = L, true_vals = true_vals, err_Cs = err_Cs)
beta_samp = samps['beta']
eta_samp = samps['eta']
